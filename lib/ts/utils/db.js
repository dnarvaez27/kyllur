...
;
class Functions {
    static async get({ db, collection_name, query, args = { limit: 10000, sort: {} } }) {
    ...
    static async createOne({ db, collection_name, args: { data } }) {
        return await (db.collection(collection_name)
            .insertOne(data));
    }
    static async createMany({ db, collection_name, args: { data } }) {
        return await (db.collection(collection_name)
            .insertMany(data));
    }
    ...
    static async updateOne({ db, collection_name, query, args: { values } }) {
        return await (db.collection(collection_name)
            .updateOne(query, values));
    }
    static async updateMany({ db, collection_name, query, args: { values } }) {
        return await (db.collection(collection_name)
            .updateMany(query, values));
    }
    static async join({ db, collection_name, args: { aggregate } }) {
        return await (db.collection(collection_name)
            .aggregate(aggregate)
            .toArray());
    }
...
__decorate([
    utils_2.decorators.interceptAsync({ before: (_, args) => console.log(`FUN: CREATE_ONE(${args[0].args})`) }),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], Functions, "createOne", null);
__decorate([
    utils_2.decorators.interceptAsync({ before: (_, args) => console.log(`FUN: CREATE_MANY(${args[0].args})`) }),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
...
__decorate([
    utils_2.decorators.interceptAsync({ before: (_, args) => console.log(`FUN: JOIN(${args[0].args})`) }),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], Functions, "join", null);
